...
// unsigned short B = 1<<4; // block size in bytes = 16 bytes
...
void accessData (
    ...
    if(cache -> front == NULL){
      ...
      cache -> front = malloc(sizeof(cache_line_t));
      (cache -> front) -> tag = tag;
      (cache -> front) -> next_cache_line = NULL;
    ...
    if ( cache->occupancy == E ) {
        ...
        cache_line_t* buff = cache -> front;
        cache -> front = (cache -> front) -> next_cache_line;
        free(buff);
        (cache -> back) -> next_cache_line = malloc(sizeof(cache_line_t));
        ((cache -> back) -> next_cache_line) -> tag = -1;
        ((cache -> back) -> next_cache_line) -> next_cache_line = NULL;
